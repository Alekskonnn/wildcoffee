...
<script lang="ts">
	...
	const scale = $derived(boardLayout.scale);
	const position = $derived({
		x:
			boardLayout.x -
			boardLayout.renderWidth * 0.5 -
			panelSizes.width * scale -
			SYMBOL_SIZE * 0.7 * scale,
		y: boardLayout.y - boardLayout.renderHeight * 0.5,
	});
